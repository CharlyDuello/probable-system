CLASS zcl_csd_order_overview_view DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS: call_popup_edit_status
      ...
      constructor
        IMPORTING
          !io_cntrl TYPE REF TO zcl_csd_order_overview_cntrl OPTIONAL ,
      call_order_overview ,
      order_overview_pai
        RAISING
          zcx_csd_webshop_exception_new ,
...
CLASS ZCL_CSD_ORDER_OVERVIEW_VIEW IMPLEMENTATION.


  METHOD call_order_overview.
    CALL FUNCTION 'ZCSD_ORDER_VIEW'
      EXPORTING
        im_web_shop = me.
  ENDMETHOD.
  ...
  METHOD call_popup_edit_amount.
    TRY.
        CALL FUNCTION 'ZCSD_ORDER_EDIT_QUANT'
          EXPORTING
            im_web_shop = me.
      CATCH cx_root .
  ...
  METHOD call_popup_edit_status.
    TRY.
        CALL FUNCTION 'ZCSD_STATUS_POPUP'
          EXPORTING
            im_web_shop = me.
      CATCH cx_root .
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new.
  ...
  METHOD call_position_overview.
    TRY.
        CALL FUNCTION 'ZCSD_POSITIONS_OVERVIEW'
          EXPORTING
            im_web_shop = me.
      CATCH cx_root .
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new.
  ...
  METHOD order_overview_pai.
    ...
    TRY.
        ...
        CASE sy-ucomm.
          ...
          WHEN 'SUCHEN'. "Butcsd Suchen des Dynpros (Funktionscode 1000 in Elementliste)
            " Markierte Zeile lesen mit Butcsd
            me->mo_cntrl->on_position( ).
          WHEN 'REFRESH'. "Butcsd Refresh
            me->mo_cntrl->on_refresh( ).
          WHEN 'DELETE'. "Butcsd Bestellung Löschen
            me->mo_cntrl->on_delete( ).
          WHEN OTHERS.
            MESSAGE  i023(zcsd_web_shop) INTO DATA(ls_msg).
  ...
  METHOD position_overview_pai.
    ...
    TRY.
        CASE sy-ucomm.
          ...
          WHEN OTHERS.
            MESSAGE  e023(zcsd_web_shop) INTO DATA(ls_msg).
            RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDCASE.
      CATCH zcx_csd_webshop_exception_new INTO DATA(e_text).
        MESSAGE e_text->get_text( ) TYPE 'S' DISPLAY LIKE 'E'.
