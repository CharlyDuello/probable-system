CLASS zcl_csd_order_overview_cntrl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      tty_bestellungen TYPE STANDARD TABLE OF zcsd_order .

    METHODS: on_edit_status ,
      on_edit_menge ,
      on_delete RAISING zcx_csd_webshop_exception_new,
      on_order_overview_pbo RAISING zcx_csd_webshop_exception_new,
      on_start ,
      constructor
        IMPORTING
                  !iv_customer_number TYPE zcsd_kd_numr_de
                  !iv_order_number    TYPE zcsd_order_number
                  !iv_status          TYPE zcsd_status
                  !iv_filter          TYPE i
                  !io_log             TYPE REF TO zcl_csd_webshop_log
                  !ir_order           TYPE zcsd_order OPTIONAL
      ...
      on_position RAISING zcx_csd_webshop_exception_new /auk/cx_vc,
      on_positions_overview_pbo RAISING zcx_csd_webshop_exception_new,
      on_refresh ,
      on_delete_position RAISING zcx_csd_webshop_exception_new,
      on_edit
        IMPORTING
          !iv_wert TYPE any .
  PROTECTED SECTION.
  PRIVATE SECTION.
    ...
    DATA: mo_best_container              TYPE REF TO cl_gui_custom_container,
          ...
          mo_web_shop_model              TYPE REF TO zcl_csd_order_overview_model,
          mv_order_number                TYPE zcsd_order_number,
          mv_status                      TYPE zcsd_status,
          mo_web_shop_view               TYPE REF TO zcl_csd_order_overview_view,
          mv_customer_number             TYPE zcsd_kd_numr_de,
          mo_alv_grid_order_overview     TYPE REF TO cl_gui_alv_grid,
          ms_positions                   TYPE ZCSD_S_ORDER,
    ...
    METHODS: dequeue_zcsd_order ,
      enqueue_zcsd_order
        RETURNING
          VALUE(rv_enqueue_ok) TYPE i ,
      create_alv_uebersicht RAISING zcx_csd_webshop_exception_new,
      search_selected_order
        RETURNING
                  VALUE(rs_selected_order) TYPE ZCSD_S_ORDER
        RAISING   zcx_csd_webshop_exception_new,
      search_selected_position
        RETURNING
                  VALUE(rs_order_positions) TYPE zcsd_order
        RAISING   zcx_csd_webshop_exception_new,
      create_alv_position RAISING zcx_csd_webshop_exception_new,
      refresh_position RAISING zcx_csd_webshop_exception_new,
      on_double_click_edit_menge
        FOR EVENT double_click OF cl_gui_alv_grid ,
      butcsd_toolbar_order
...
CLASS ZCL_CSD_ORDER_OVERVIEW_CNTRL IMPLEMENTATION.


  METHOD butcsd_toolbar_order.
    ...
    CONSTANTS: lc_function_code TYPE char70 VALUE 'DELETE',
               ...
               lc_butcsd_text   TYPE char40 VALUE 'Bestellung Löschen'.
  ...
  METHOD butcsd_toolbar_position.

    DATA ls_butcsd_position TYPE stb_butcsd.

    CONSTANTS: lc_function_code TYPE char70 VALUE 'DELETE',
               lc_quickinfo     TYPE char30 VALUE 'Löschen einer Position',
               lc_disabled      TYPE char1  VALUE ' ',
               lc_butcsd_text   TYPE char40 VALUE 'Position Löschen'.

    "Einfügen eines Seperators (Senkrechter Strich) zum Absetzen von anderen Butcsds
    CLEAR ls_butcsd_position.
    ls_butcsd_position-butn_type = 3. "Seperator
    APPEND ls_butcsd_position TO e_object->mt_toolbar.

    "Einfügen des Delete-Butcsds
    CLEAR ls_butcsd_position.

    ls_butcsd_position = VALUE stb_butcsd(  function = lc_function_code
                                                icon = icon_cancel
                                           quickinfo = lc_quickinfo
                                            disabled = lc_disabled
                                                text = lc_butcsd_text ).
  ...
  METHOD constructor.
    ...
    me->mo_web_shop_model = NEW zcl_csd_order_overview_model( NEW zcl_csd_webshop_log( iv_object = 'ZCSD_' iv_suobj = 'ZCSD_' ) ).

    "Instanziierung der View
    me->mo_web_shop_view = NEW zcl_csd_order_overview_view( io_cntrl = me ).
    ...
    IF me->mo_web_shop_model IS NOT BOUND OR me->mo_web_shop_view IS NOT BOUND.
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new
        EXPORTING
          textid = zcx_csd_webshop_exception_new=>object_not_found.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD create_alv_position.
    ...
    IF mo_pos_container IS INITIAL.
      ...
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new
          EXPORTING
            textid = zcx_csd_webshop_exception_new=>alv_not_able_to_create.
      ENDIF.
    ELSE.
  ...
  METHOD dequeue_zcsd_order.

    CALL FUNCTION 'DEQUEUE_EZCSD_ORDER'
      EXPORTING
        mode_zcsd_order = 'E'
  ...
  METHOD enqueue_zcsd_order.

    "Sperre setzen
    CALL FUNCTION 'ENQUEUE_EZCSD_ORDER'
      EXPORTING
        mode_zcsd_order = 'E'
        order_number        = me->ms_positions-order_number
      EXCEPTIONS
  ...
  METHOD on_delete.
    ...
    CONSTANTS: lc_text           TYPE char90 VALUE 'Sind Sie sicher das Sie die Bestellung löschen wollen?'    ##no_text,
               ...
               lc_butcsd1_sperre TYPE char15 VALUE 'OK'                                                        ##no_text.

    TRY.
        "Sperre setzen
        lv_sysubrc = me->enqueue_zcsd_order( ).
        ...
        IF lv_sysubrc = 0.
          ...
          IF lv_btn = 1.
            ...
            IF ls_selected_order IS INITIAL.
              MESSAGE i034(zcsd_web_shop) INTO DATA(ls_msg).
        ...
        ELSEIF lv_sysubrc = 1.
          ...
          DATA(lv_butcsd) = /auk/cl_msgbox=>show_msgbox( im_text    = lc_text_sperre
                                                         im_kind    = lc_kind_sperre
                                                         im_butcsd1 = lc_butcsd1_sperre ).
      ...
      CATCH /auk/cx_vc.
        MESSAGE e082(zcsd_web_shop) INTO DATA(lv_msg).
        me->mo_log->add_msg_from_sys( ).
        me->mo_log->safe_log( ).
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDTRY.
  ...
  METHOD on_order_overview_pbo.
    ...
    SET PF-STATUS '9001'           OF PROGRAM 'SAPLZCSD_ORDER_OVERVIEW'.
    SET TITLEBAR 'UEBERSICHT9001'  OF PROGRAM 'SAPLZCSD_ORDER_OVERVIEW'.
    ...
    IF me->mo_web_shop_model->mt_order_view IS INITIAL.
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new
        EXPORTING
          textid = zcx_csd_webshop_exception_new=>order_not_found.
      ...
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new
          EXPORTING
            textid = zcx_csd_webshop_exception_new=>alv_not_able_to_create.
      ENDIF.
      SET HANDLER me->butcsd_toolbar_order
                  me->on_double_click
...
*    lv_sy_subrc = me->enqueue_zcsd_bestellung( ).

        IF me->enqueue_zcsd_order( ) = 0.
          "Rufe View Positionsübersicht auf
          me->mo_web_shop_view->call_position_overview( ).

        ELSEIF me->enqueue_zcsd_order( ) = 1.
        ...
        ELSEIF me->enqueue_zcsd_order( ) = 2 OR me->enqueue_zcsd_order( ) = 3.
  ...
  METHOD on_positions_overview_pbo.
    ...
    SET PF-STATUS '9002'                OF PROGRAM 'SAPLZCSD_ORDER_OVERVIEW'.
    SET TITLEBAR 'POSITIONSUEBERSICHT'  OF PROGRAM 'SAPLZCSD_ORDER_OVERVIEW'.
    ...
    IF me->mt_positions IS INITIAL.
      ...
      MESSAGE i020(zcsd_web_shop) INTO DATA(ls_msg).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
  ...
  METHOD search_selected_order.
    ...
    LOOP AT it_sel_rows ASSIGNING  FIELD-SYMBOL(<lv_sel_rows>).
      ...
      IF sy-subrc NE 0.
        MESSAGE: e024(zcsd_web_shop) INTO DATA(ls_msg).
        me->mo_log->add_msg_from_sys( ).
        me->mo_log->safe_log( ).
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
      ENDIF.
  ...
  METHOD search_selected_position.
    ...
    IF it_sel_rows IS INITIAL.
      rs_order_positions = VALUE zcsd_order( mt_positions[ 1 ] OPTIONAL ).
    ELSE.
      "Falls User eine Position ausgewählt hat
      rs_order_positions = VALUE zcsd_order( mt_positions[ it_sel_rows[ 1 ]-index ] OPTIONAL ).
    ENDIF.

    "Falls ein keine Position in der Struktur ist
    IF rs_order_positions IS INITIAL.
      MESSAGE i036(zcsd_web_shop) INTO DATA(ls_msg).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
