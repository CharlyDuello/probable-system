CLASS zcl_csd_homescreen_model DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA:
      mt_order                 TYPE zcsd_tt_order,
      mt_articles              TYPE TABLE OF zcsd_article WITH KEY mandt article_number,
      mt_articles_string_table TYPE TABLE OF string,
      mt_searched_articles     TYPE match_result_tab,
      mt_cart_order            TYPE TABLE OF zcsd_order,
      mt_articles_out          TYPE TABLE OF zcsd_article,
      mt_cart                  TYPE TABLE OF zcsd_s_cart,
    ...
    METHODS: search_entries_to_new_table
      ...
      remove_item_from_cart
        IMPORTING
          !iv_article_number TYPE zcsd_article_number ,
      constructor
        IMPORTING
          !io_home_screen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl
          !iv_customer_number        TYPE zcsd_customer_number
          !io_log                    TYPE REF TO zcl_csd_webshop_log,
      add_to_cart
        IMPORTING
                  !iv_number_of_articles TYPE zcsd_order_amount
                  !is_article            TYPE zcsd_article
        RAISING   zcx_csd_webshop_exception_new,
      ...
      return_address
        RETURNING
          VALUE(rv_address) TYPE zcsd_s_adress ,
      get_email_address_of_customer
        RETURNING
          VALUE(rv_email) TYPE zcsd_email ,
      set_customer_email
        IMPORTING
      ...
      get_open_order_from_mt_order
        RETURNING
          VALUE(rt_openorder) TYPE zcsd_tt_order ,
      get_done_order_from_mt_order
        RETURNING
          VALUE(rt_done_order) TYPE zcsd_tt_order ,
      get_order
        IMPORTING
          !iv_order_number TYPE tv_nodekey
        RETURNING
          VALUE(rt_order)  TYPE zcsd_tt_order ,
      delete_position
        IMPORTING
      ...
      add_up_item
        IMPORTING
          !iv_article_number TYPE zcsd_article_number
          !iv_quantity       TYPE zcsd_order_amount
        RETURNING
          VALUE(rv_quantity) TYPE zcsd_order_amount,
      check_if_order_exist
        IMPORTING is_selected_row TYPE zcsd_order
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      mc_status_inactive    TYPE zcsd_status VALUE 'IN' ##NO_TEXT,  "Bestellung Inaktive
      mc_status_completed   TYPE zcsd_status VALUE 'AB' ##NO_TEXT,   "Bestellung Abgeschlossen
      mc_status_in_progress TYPE zcsd_status VALUE 'IB' ##NO_TEXT,   "Bestellung in Bearbeitung
      mc_status_ordered     TYPE zcsd_status VALUE 'BE' ##NO_TEXT,   "Bestellung Bestellt
      mc_range_nr           TYPE nrnr VALUE '01' ##NO_TEXT.

    DATA:
      mo_home_screen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl,
      mv_customernumber         TYPE zcsd_customer_number,
      ms_order_address          TYPE zcsd_s_adress,
      mv_customer_email         TYPE zcsd_email.
...
CLASS ZCL_CSD_HOMESCREEN_MODEL IMPLEMENTATION.
  ...
  METHOD get_ordernumber.
    ...
    CALL FUNCTION 'NUMBER_GET_NEXT'
      ...
      EXCEPTIONS
        ...
        OTHERS                  = 8.
    IF sy-subrc <> 0.
      MESSAGE i046(zcsd_web_shop) WITH sy-subrc.
    ENDIF.
  ...
  METHOD order_cart.
    ...
    INSERT zcsd_order FROM TABLE mt_cart_order.
  ...
  METHOD select_articles_as_char_fields.
    ...
    IF sy-subrc <> 0.
      MESSAGE i058(zcsd_web_shop) INTO DATA(ls_msg).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
