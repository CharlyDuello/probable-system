...
CLASS ltc_customer_model DEFINITION FOR TESTING RISK LEVEL HARMLESS.
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: m_cut               TYPE REF TO zcl_csd_customer_login_model,
    ...
    CONSTANTS: lc_customer_number_1 TYPE zcsd_customer_number VALUE '1',
               lc_customer_number_2 TYPE zcsd_customer_number VALUE '2',
               lc_name_1            TYPE zcsd_name VALUE 'Meyer',
               lc_name_2            TYPE zcsd_name VALUE 'Müller',
               lc_mail_1            TYPE zcsd_email VALUE 'test@test.de',
               lc_mail_2            TYPE zcsd_email VALUE 'unit-test@test.de',
               lc_password_1        TYPE zcsd_password VALUE '1234',
               lc_password_2        TYPE zcsd_password VALUE 'ABCD'.
...
CLASS ltc_customer_model IMPLEMENTATION.
  METHOD setup.
    ...
    m_cut = NEW zcl_csd_customer_login_model( io_cntrl = login_cntrl io_log = mo_log ).
  ...
  METHOD check_email_is_available_true.
    ...
    "given
    DATA: lv_mail TYPE zcsd_email VALUE 'new@test.de'.
    TRY.
        "when
  ...
  METHOD check_email_is_available_false.
    "given
    DATA: lv_mail TYPE zcsd_email VALUE 'test@test.de'.
    TRY.
        "when
        m_cut->check_if_email_is_available( iv_email = lv_mail ).
  ...
  METHOD check_passwort_no_account_exc.
    "given
    DATA: lv_password TYPE zcsd_password VALUE '1234',
          lv_mail     TYPE zcsd_email VALUE 'Test2@test.de'.
  ...
  METHOD check_passwort_false.
    "given
    DATA: lv_password TYPE zcsd_password VALUE '4321',
          lv_mail     TYPE zcsd_email VALUE 'Test@test.de'.
  ...
  METHOD check_pw_and_account_pass.
    ...
    "given
    DATA: lv_password TYPE zcsd_password VALUE lc_password_1,
          lv_mail     TYPE zcsd_email VALUE lc_mail_1.
  ...
  METHOD check_get_customer_number.
    "given
    DATA: lv_customer_number TYPE zcsd_customer_number,
          lv_mail            TYPE zcsd_email VALUE 'test@test.de'.
  ...
  METHOD get_customer_number_exc_exist.
    "given
    DATA: lv_customer_number TYPE zcsd_customer_number,
          lv_mail            TYPE zcsd_email VALUE 'nicht-vorhanden@test.de'.
