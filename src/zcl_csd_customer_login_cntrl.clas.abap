CLASS zcl_csd_customer_login_cntrl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
    ...
    METHODS:
      ...
      encrypt_password
        IMPORTING
          !iv_password               TYPE zcsd_password
        RETURNING
          VALUE(rv_password_as_hash) TYPE zcsd_password ,
      on_pbo_login_screen
        RAISING zcx_csd_webshop_exception_new .
  PROTECTED SECTION.
  PRIVATE SECTION.

    DATA: mv_customer_email         TYPE zcsd_email,
          mo_customer_login_view    TYPE REF TO zcl_csd_customer_login_view,
          mo_customer_login_model   TYPE REF TO zcl_csd_customer_login_model,
          mo_customer_register_view TYPE REF TO zcl_csd_customer_register_view,
...
CLASS ZCL_CSD_CUSTOMER_LOGIN_CNTRL IMPLEMENTATION.
  ...
  METHOD on_confirm_login.
    ...
    IF iv_email IS INITIAL OR iv_password IS INITIAL.
      MESSAGE i040(zcsd_web_shop) INTO DATA(ls_msg).
    ...
    TRY.
        ...
        me->mo_home_screen_cntrl = NEW zcl_csd_homescreen_cntrl( io_login_cntrl = me
  ...
  METHOD on_confirm_registration.
    ...
    CONSTANTS: lc_text_password        TYPE char90 VALUE   'Die eingegebenen Passwörter stimmen nicht überein!',
               ...
               lc_butcsd               TYPE char15 VALUE   'Okay'.

    DATA: lv_butcsd TYPE i.
    ...
    TRY.
        IF is_register_data-city            IS INITIAL OR
         ...
         is_register_data-zip_code        IS INITIAL.
          ...
          lv_butcsd =  /auk/cl_msgbox=>show_msgbox( im_text    = lc_text_field
                                                       im_kind    = lc_kind
                                                       im_butcsd1 = lc_butcsd ).

        ELSEIF is_register_data-password <> is_register_data-password_repeat.
          "Passwords are not the same
          lv_butcsd =  /auk/cl_msgbox=>show_msgbox( im_text = lc_text_password
                                                       im_kind    = lc_kind
                                                       im_butcsd1 = lc_butcsd ).
        ...
        ELSE.
          TRY.
              ...
              DATA(lv_butcsd_pop_up_success) =  /auk/cl_msgbox=>show_msgbox( im_text    = lc_text_successful_save
                                                                             im_butcsd1 = lc_butcsd ).
            CATCH zcx_csd_webshop_exception_new INTO DATA(e_text).
              MESSAGE e_text->get_text( ) TYPE 'S' DISPLAY LIKE 'E'.
          ENDTRY.
  ...
  METHOD on_pbo_login_screen.
    ...
    IF me->mo_home_screen_cntrl IS BOUND.
      MESSAGE i056(zcsd_web_shop) INTO DATA(ls_msg).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
  ...
  METHOD on_register.
    IF me->mo_customer_register_view IS NOT BOUND.
      me->mo_customer_register_view = NEW zcl_csd_customer_register_view( io_login_cntrl = me ).
  ...
  METHOD start.
    ...
    IF me->mo_customer_login_view IS NOT BOUND.
      mo_customer_login_view = NEW zcl_csd_customer_login_view( io_customer_login_cntrl = me ).
    ENDIF.

    IF me->mo_customer_login_model IS NOT BOUND.
      mo_customer_login_model = NEW zcl_csd_customer_login_model( io_cntrl = me io_log = mo_log ).
    ENDIF.
    TRY.
