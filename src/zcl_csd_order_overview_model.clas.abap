  CLASS zcl_csd_order_overview_model DEFINITION
    PUBLIC
    FINAL
    CREATE PUBLIC.
    ...
    PUBLIC SECTION.
      ...
      TYPES: t_table  TYPE STANDARD TABLE OF zcsd_order WITH DEFAULT KEY,
             tty_view TYPE STANDARD TABLE OF ZCSD_S_ORDER WITH DEFAULT KEY.
      ...
      METHODS: constructor IMPORTING io_log TYPE REF TO zcl_csd_webshop_log,
        delete_position IMPORTING is_position TYPE zcsd_order
                        RAISING   zcx_csd_webshop_exception_new,
        edit_postition  IMPORTING iv_wert     TYPE any
                                  is_position TYPE zcsd_order
                        RAISING   zcx_csd_webshop_exception_new,
        delete_order    IMPORTING iv_order_number TYPE zcsd_order_number
                        RAISING   zcx_csd_webshop_exception_new,
        get_orders      RETURNING VALUE(rt_orders) TYPE t_table,
        get_information IMPORTING VALUE(iv_filter)          TYPE i OPTIONAL
                                  VALUE(iv_customer_number) TYPE zcsd_kd_numr_de OPTIONAL
                                  VALUE(iv_order_number)    TYPE zcsd_order_number OPTIONAL
                                  VALUE(iv_status)          TYPE zcsd_status OPTIONAL
                        RAISING   zcx_csd_webshop_exception_new,
        select_with_condition  IMPORTING iv_condition             TYPE any OPTIONAL
                                         iv_condition_description TYPE string
                               RAISING   zcx_csd_webshop_exception_new,
        get_positions   IMPORTING iv_order_number TYPE zcsd_order_number,
    ...
    PRIVATE SECTION.
      ...
      METHODS: change_structur_from_value IMPORTING iv_wert     TYPE any
                             CHANGING  cs_position TYPE zcsd_order
                             RAISING   zcx_csd_webshop_exception_new,
        add_to_view   IMPORTING is_order       TYPE zcsd_order
                                iv_total_price TYPE p
                      CHANGING  cs_view        TYPE ZCSD_S_ORDER,
...
CLASS ZCL_CSD_ORDER_OVERVIEW_MODEL IMPLEMENTATION.
    ...
    METHOD get_order_overview.
      ...
      DATA: lt_orders        TYPE TABLE OF zcsd_order,
            ls_orders        TYPE          zcsd_order,
            ls_zwischen      TYPE          zcsd_order,
            lv_price_article TYPE          zcsd_price,
            lv_total_price   TYPE          p,
            lt_view          TYPE TABLE OF ZCSD_S_ORDER,
            ls_view          TYPE          ZCSD_S_ORDER,
            lv_counter       TYPE          i.
