...
CLASS zcl_csd_homescreen_model DEFINITION LOCAL FRIENDS ltc_homescreen_model_orders.
CLASS ltc_homescreen_model_orders DEFINITION FOR TESTING RISK LEVEL HARMLESS.
  PUBLIC SECTION.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: m_cut                 TYPE REF TO zcl_csd_homescreen_model,
          lt_order_data         TYPE TABLE OF zcsd_order,
          l_exception_occured   TYPE abap_bool,
          homescreen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl,
          login_cntrl      TYPE REF TO zcl_csd_customer_login_cntrl,
          mo_log                TYPE REF TO zcl_csd_webshop_log.

    CONSTANTS: mc_customer_number    TYPE zcsd_customer_number VALUE '1',
               mc_order_number_1     TYPE zcsd_order_number VALUE  '1',
               mc_order_number_2     TYPE zcsd_order_number VALUE  '2',
               mc_position_number_1  TYPE zcsd_position_number VALUE '1',
               mc_position_number_2  TYPE zcsd_position_number VALUE '2',
               mc_order_amount_1     TYPE zcsd_order_amount VALUE 01,
               mc_order_amount_2     TYPE zcsd_order_amount VALUE 02,
               mc_order_status       TYPE zcsd_status VALUE 'BE',
               mc_order_value        TYPE zcsd_order_amount VALUE '100',
               mc_status_in_progress TYPE zcsd_status VALUE 'IB',
               mc_status_completet   TYPE zcsd_status VALUE 'AB'.
...
CLASS ltc_homescreen_model_orders IMPLEMENTATION.
  ...
  METHOD class_setup.
    m_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'zcsd_order' ) ) ).
  ENDMETHOD.
  ...
  METHOD setup.
    ...
    login_cntrl = NEW zcl_csd_customer_login_cntrl( io_log = mo_log  ).
    mo_log = NEW zcl_csd_webshop_log( iv_object = 'ZCSD_' iv_suobj = 'ZCSD_' ).
    homescreen_cntrl = NEW zcl_csd_homescreen_cntrl( io_login_cntrl = login_cntrl iv_customer_number =  mc_customer_number iv_email =  'test@web.de' io_log = mo_log ).

    m_environment->clear_doubles( ).

    m_cut = NEW zcl_csd_homescreen_model( io_home_screen_cntrl = homescreen_cntrl iv_customer_number = mc_customer_number io_log = mo_log ).
  ...
  METHOD check_add_to_cart.
    "given
    DATA: lv_article            TYPE zcsd_article,
          lv_number_of_articles TYPE zcsd_order_amount.
  ...
  METHOD check_all_orders_from_customer.
    DATA: mt_orders TYPE zcsd_tt_order.
    TRY.
        "when
    ...
    ELSE.
      "In Tabelle zcsd_order werden einzelne Bestellpositionen auch als einzelne  Bestellung gespeichert
  ...
  METHOD check_get_done_order.
    DATA: lv_done_orders TYPE zcsd_tt_order.
    TRY.
        "when
        m_cut->get_all_orders_from_customer(  ).
      CATCH zcx_csd_webshop_exception_new .
  ...
  METHOD check_if_order_exist_true.
    DATA: lv_exist          TYPE boolean,
          ls_order_to_check TYPE zcsd_order.
    "given
...
CLASS zcl_csd_homescreen_model DEFINITION LOCAL FRIENDS ltc_homescreen_model_artikel.
CLASS ltc_homescreen_model_artikel DEFINITION FOR TESTING RISK LEVEL HARMLESS.
  PUBLIC SECTION.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: m_cut                 TYPE REF TO zcl_csd_homescreen_model,
          lt_article_data       TYPE TABLE OF zcsd_article,
          l_exception_occured   TYPE abap_bool,
          homescreen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl,
          login_cntrl      TYPE REF TO zcl_csd_customer_login_cntrl,
          mo_log                TYPE REF TO zcl_csd_webshop_log.
    ...
    CONSTANTS: mc_customer         TYPE zcsd_customer_number VALUE '1',
               mc_article_number_1 TYPE zcsd_article_number VALUE '1',
               mc_article_number_2 TYPE zcsd_article_number VALUE '2',
               mc_article_number_3 TYPE zcsd_article_number VALUE '3',
               mc_description_1    TYPE zcsd_description VALUE 'Bildschirm',
               mc_description_2    TYPE zcsd_description VALUE 'Tastatur',
               mc_description_3    TYPE zcsd_description VALUE 'Maus',
               mc_currency         TYPE zcsd_currency VALUE 'Euro'.
...
CLASS ltc_homescreen_model_artikel IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_log = NEW zcl_csd_webshop_log( iv_object = 'ZCSD_' iv_suobj = 'ZCSD_' ).
    login_cntrl = NEW zcl_csd_customer_login_cntrl( io_log = mo_log  ).
    homescreen_cntrl = NEW zcl_csd_homescreen_cntrl( io_login_cntrl = login_cntrl iv_customer_number =  mc_customer iv_email =  'test@web.de' io_log = mo_log ).

    m_environment->clear_doubles( ).

    m_cut = NEW zcl_csd_homescreen_model( io_home_screen_cntrl = homescreen_cntrl iv_customer_number = mc_customer io_log = mo_log ).
    m_environment->insert_test_data( EXPORTING i_data = lt_article_data ).
  ENDMETHOD.

  METHOD class_setup.
    m_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'zcsd_article' ) ) ).
  ENDMETHOD.

  METHOD check_select_articles.
    DATA: lt_articles TYPE TABLE OF zcsd_article.
...
CLASS zcl_csd_homescreen_model DEFINITION LOCAL FRIENDS ltc_homescreen_model_adress.
CLASS ltc_homescreen_model_adress DEFINITION FOR TESTING RISK LEVEL HARMLESS.
  PUBLIC SECTION.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: m_cut                 TYPE REF TO zcl_csd_homescreen_model,
          lt_adress_data        TYPE TABLE OF zcsd_order_ad,
          l_exception_occured   TYPE abap_bool,
          homescreen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl,
          login_cntrl      TYPE REF TO zcl_csd_customer_login_cntrl,
          mo_log                TYPE REF TO zcl_csd_webshop_log.
    ...
    CONSTANTS: mc_customer_number TYPE zcsd_customer_number VALUE '1',
               ...
               mc_street_1        TYPE zcsd_street VALUE 'Schlossallee',
               mc_street_2        TYPE zcsd_street VALUE 'Parkstrasse',
               mc_zip_code        TYPE zcsd_postalcode VALUE 97070,
               mc_city            TYPE zcsd_city VALUE 'Würzburg'.
...
CLASS ltc_homescreen_model_adress IMPLEMENTATION.
  ...
  METHOD class_setup.
    m_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'zcsd_order_ad' ) ) ).
  ENDMETHOD.

  METHOD check_insert_address_of_order.
    DATA: lt_adress TYPE TABLE OF zcsd_order_ad.
    ...
    SELECT SINGLE street
        FROM zcsd_order_ad INTO @DATA(ls_adress)
...
CLASS zcl_csd_homescreen_model DEFINITION LOCAL FRIENDS ltc_homescreen_model_customer.
CLASS ltc_homescreen_model_customer DEFINITION FOR TESTING RISK LEVEL HARMLESS.
  PUBLIC SECTION.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA: m_cut                 TYPE REF TO zcl_csd_homescreen_model,
          lt_adress_data        TYPE TABLE OF zcsd_customer,
          l_exception_occured   TYPE abap_bool,
          homescreen_cntrl TYPE REF TO zcl_csd_homescreen_cntrl,
          login_cntrl      TYPE REF TO zcl_csd_customer_login_cntrl,
          mo_log                TYPE REF TO zcl_csd_webshop_log.

    CONSTANTS: mc_customer_number_1 TYPE zcsd_customer_number VALUE '1',
               mc_customer_number_2 TYPE zcsd_customer_number VALUE '2',
               mc_name_1            TYPE zcsd_name VALUE 'Meyer',
               mc_name_2            TYPE zcsd_name VALUE 'Müller',
               mc_street_1          TYPE zcsd_street VALUE 'Schlossallee',
               mc_street_2          TYPE zcsd_street VALUE 'Parkstrasse',
               mc_house_number      TYPE zcsd_house_nr VALUE 1,
               mc_zip_code          TYPE zcsd_postalcode VALUE 97070,
               mc_city              TYPE zcsd_city VALUE 'Würzburg'.
...
CLASS ltc_homescreen_model_customer IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_log = NEW zcl_csd_webshop_log( iv_object = 'ZCSD_' iv_suobj = 'ZCSD_' ).
    login_cntrl = NEW zcl_csd_customer_login_cntrl( io_log = mo_log ).
    homescreen_cntrl = NEW zcl_csd_homescreen_cntrl( io_login_cntrl = login_cntrl iv_customer_number =  mc_customer_number_2 iv_email =  'test@web.de' io_log = mo_log ).

    m_environment->clear_doubles( ).

    m_cut = NEW zcl_csd_homescreen_model( io_home_screen_cntrl = homescreen_cntrl iv_customer_number = mc_customer_number_2 io_log = mo_log ).
    m_environment->insert_test_data( EXPORTING i_data = lt_adress_data ).
  ENDMETHOD.

  METHOD class_setup.
    m_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'zcsd_customer' ) ) ).
  ENDMETHOD.

  METHOD check_get_adress_of_customer.

    DATA: ls_order_adress TYPE zcsd_s_adress.
    TRY.
        "when
