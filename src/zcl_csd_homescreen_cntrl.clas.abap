CLASS zcl_csd_homescreen_cntrl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC

  GLOBAL FRIENDS zcl_csd_customer_login_cntrl .

  PUBLIC SECTION.

    CONSTANTS lc_max_number_of_product TYPE i VALUE 50 ##NO_TEXT.
    DATA: mo_login_cntrl      TYPE REF TO zcl_csd_customer_login_cntrl,
          mo_home_screen_view TYPE REF TO zcl_csd_homescreen_view,
          mt_order_to_show    TYPE zcsd_tt_order,
          mo_log              TYPE REF TO zcl_csd_webshop_log.

    METHODS: add_butcsds_show_position
      ...
      get_quantity
        RETURNING
                  VALUE(rv_quantity) TYPE zcsd_order_amount
      ...
      search_selected_product
        RETURNING
          VALUE(rs_selected_product) TYPE zcsd_article ,
      on_pbo_cart ,
      on_back ,
      on_leave
        RAISING zcx_csd_webshop_exception_new ,
      ...
      on_confirm_address
        IMPORTING
          !is_order_address TYPE zcsd_s_adress ,
      ...
      constructor
        IMPORTING
          !io_login_cntrl TYPE REF TO zcl_csd_customer_login_cntrl
          !iv_customer_number  TYPE zcsd_customer_number
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mo_home_screen_model          TYPE REF TO zcl_csd_homescreen_model,
    ...
    METHODS:
      ...
      search_selected_item_in_cart
        RETURNING
          VALUE(rs_selected_item) TYPE zcsd_s_cart ,
      ...
      delete_node_and_items
        IMPORTING
          !is_position TYPE zcsd_order ,
      edit_quantity_order_position ,
      on_butcsds_show_position
          FOR EVENT user_command OF cl_gui_alv_grid
        IMPORTING
          !e_ucomm ,
...
CLASS ZCL_CSD_HOMESCREEN_CNTRL IMPLEMENTATION.


  METHOD add_btn_to_toolbar_add_to_cart.

    DATA ls_butcsd TYPE stb_butcsd.
    CONSTANTS: lc_function_code TYPE char70 VALUE 'ADD',
               lc_quickinfo     TYPE char30 VALUE 'Add to cart',
               lc_disabled      TYPE char1  VALUE ' ',
               lc_butcsd_text   TYPE char40 VALUE 'Zum Warenkorb Hinzufügen'.

    "Einfügen eines Seperators (Senkrechter Strich) zum Absetzen von anderen Butcsds
    CLEAR ls_butcsd.
    ls_butcsd-butn_type = 3. "Seperator
    APPEND ls_butcsd TO e_object->mt_toolbar.

    "Einfügen des Butcsds
    CLEAR ls_butcsd.

    ls_butcsd = VALUE stb_butcsd(  function = lc_function_code
                                       icon = icon_checked
                                  quickinfo = lc_quickinfo
                                   disabled = lc_disabled
                                       text = lc_butcsd_text ).
    "Hinzufügen des Butcsds zur Toolbar
    APPEND ls_butcsd TO e_object->mt_toolbar.
  ...
  METHOD add_btn_to_toolbar_edit_quan.
    ...
    CONSTANTS: lc_function_code TYPE char70 VALUE 'EDIT',
               ...
               lc_disabled      TYPE char1  VALUE ' ',
               lc_butcsd_text   TYPE char40 VALUE 'Menge Bearbeiten'.
  ...
  METHOD add_butcsds_show_position.

    DATA: lt_butcsds TYPE TABLE OF stb_butcsd.

    CONSTANTS: lc_function_code_ca TYPE char70 VALUE 'CANCEL'           ##no_text,
               lc_quickinfo_ca     TYPE char30 VALUE 'Position Canceln' ##no_text,
               lc_butcsd_text_ca   TYPE char40 VALUE 'Position Canceln' ##no_text,
               lc_function_code_ed TYPE char70 VALUE 'EDIT'             ##no_text,
               lc_quickinfo_ed     TYPE char30 VALUE 'Menge bearbeiten' ##no_text,
               lc_butcsd_text_ed   TYPE char40 VALUE 'Menge bearbeiten' ##no_text.

    "Butcsd Cancel Position
    lt_butcsds = VALUE #( ( butn_type  = 3 ) "Seperator
                         ( function   = lc_function_code_ca
                           icon       = icon_cancel
                           quickinfo  = lc_quickinfo_ca
                           disabled   = abap_false
                           text       = lc_butcsd_text_ca ) ).

    "Butcsd Edit Quantity
    lt_butcsds = VALUE #( BASE lt_butcsds
                         ( butn_type  = 3 ) "Seperator
                         ( function   = lc_function_code_ed
                           icon       = icon_edit_file
                           quickinfo  = lc_quickinfo_ed
                           disabled   = abap_false
                           text       = lc_butcsd_text_ed ) ).

    APPEND LINES OF lt_butcsds TO e_object->mt_toolbar.
  ...
  METHOD create_alv_for_cart.
    ...
    TRY.
        ...
        me->mo_alv_grid_cart->set_table_for_first_display( EXPORTING  i_structure_name = 'ZCSD_S_CART'
        ...
        IF sy-subrc <> 0.
          MESSAGE i052(zcsd_web_shop) WITH sy-subrc INTO DATA(ls_msg).
  ...
  METHOD create_alv_for_home_screen.
    ...
    TRY.
        ...
        IF me->mo_container_home_screen IS BOUND AND me->mo_alv_grid_home_screen IS NOT BOUND.
          ...
          me->mo_alv_grid_home_screen->set_table_for_first_display( EXPORTING i_structure_name = 'ZCSD_ARTICLE'
          ...
          IF sy-subrc <> 0.
            MESSAGE i053(zcsd_web_shop) WITH sy-subrc INTO DATA(ls_msg).
            me->mo_log->add_msg_from_sys( ).
            me->mo_log->safe_log( ).
            RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
          ENDIF.
...
*          SET HANDLER me->handle_node_context_menu_sel FOR me->mo_tree.
        ...
        ELSE.
          "do nothing
        ENDIF.
      CATCH zcx_csd_webshop_exception_new INTO DATA(e_text).
  ...
  METHOD delete_node_and_items.
    ...
    me->mo_tree->delete_node( EXPORTING  node_key = CONV #( is_position-order_number ) " Node key
                              EXCEPTIONS failed            = 1                " General Error
                                         ...
                                         OTHERS            = 4  ).
    IF sy-subrc <> 0.
      MESSAGE i062(zcsd_web_shop) WITH sy-subrc.
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
    ENDIF.
  ...
  METHOD edit_quantity_order_position.
    ...
    TRY.
        ...
        IF lt_index_row IS INITIAL.
          MESSAGE i067(zcsd_web_shop) INTO DATA(lv_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        IF lv_quantity IS INITIAL OR lv_quantity <= 0.
          MESSAGE i066(zcsd_web_shop) INTO lv_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD get_quantity.
    ...
    IF sy-subrc <> 0.
      MESSAGE i045(zcsd_web_shop) INTO DATA(ls_msg).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
  ...
  METHOD on_add_product_to_cart.
    ...
    TRY.
        ...
        ELSEIF lv_quantity <= 0.
          MESSAGE i048(zcsd_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.

        ELSEIF lv_quantity >= lc_max_number_of_product.
          MESSAGE i049(zcsd_web_shop) INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD on_back_to_login.
    ...
    CONSTANTS: lc_text    TYPE char90 VALUE 'Wollen Sie wirklich das Kundenportal verlassen? Ihr Warenkorb geht verloren!' ##no_text,
               ...
               lc_butcsd1 TYPE char15 VALUE 'Ja'                                                                           ##no_text,
               lc_butcsd2 TYPE char15 VALUE 'Nein'                                                                         ##no_text.
    TRY.
        DATA(lv_btn) = /auk/cl_msgbox=>show_msgbox( im_text    = lc_text
                                                    im_kind    = lc_kind
                                                    im_butcsd1 = lc_butcsd1
                                                    im_butcsd2 = lc_butcsd2 ).
      CATCH  /auk/cx_vc .
        RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new.
    ENDTRY.

    IF lv_btn = 1.
      MESSAGE i056(zcsd_web_shop).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      FREE mo_login_cntrl.
      "start user login
  ...
  METHOD on_butcsds_show_position.
  ...
  METHOD on_edit_quantity.
    ...
    TRY.
        ...
        IF lv_quantity > 0 AND lv_quantity < lc_max_number_of_product.
          ...
          DATA ls_item_for_cart TYPE zcsd_article.
          DATA(ls_item) =  me->search_selected_item_in_cart( ).

          ls_item_for_cart = VALUE zcsd_article( article_number = ls_item-article_number
  ...
  METHOD on_order.
    ...
    TRY.
        CONSTANTS: lc_text       TYPE char90 VALUE 'An welche Adresse soll geliefert werden?'  ##no_text,
                   ...
                   lc_butcsd1    TYPE char15 VALUE 'Standard'                                  ##no_text,
                   lc_butcsd2    TYPE char15 VALUE 'Alternativ'                                ##no_text,
                   lc_butcsd3    TYPE char15 VALUE 'Abbrechen'                                 ##no_text,
                   lc_text_sc    TYPE char90 VALUE 'Bestellung wurde erfolgreich aufgegeben!'  ##no_text,
                   lc_kind_sc    TYPE char4  VALUE 'INFO'                                      ##no_text,
                   lc_butcsd1_sc TYPE char15 VALUE 'OK'                                        ##no_text.

        DATA(lv_btn) = /auk/cl_msgbox=>show_msgbox( im_text    = lc_text
                                                    im_kind    = lc_kind
                                                    im_butcsd1 = lc_butcsd1
                                                    im_butcsd2 = lc_butcsd2
                                                    im_butcsd3 = lc_butcsd3 ).
        ...
        IF me->mo_home_screen_model->mt_cart IS NOT INITIAL.
          ...
          IF lv_btn = 1.
            ...
            DATA(lv_butcsd) = /auk/cl_msgbox=>show_msgbox( im_text    = lc_text_sc
                                                           im_kind    = lc_kind_sc
                                                           im_butcsd1 = lc_butcsd1_sc ).
        ...
        ELSE.
          MESSAGE i050(zcsd_web_shop) INTO DATA(ls_msg).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
      CATCH zcx_csd_webshop_exception_new INTO DATA(e_text).
        MESSAGE e_text->get_text( ) TYPE 'S' DISPLAY LIKE 'E'.
  ...
  METHOD on_remove_item_from_cart.
    ...
    TRY.
        ...
        IF me->mo_home_screen_model->mt_cart IS INITIAL.
          MESSAGE i051(zcsd_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_alv_grid_cart->refresh_table_display( ).
        IF sy-subrc <> 0.
          MESSAGE i047(zcsd_web_shop) INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new.
        ENDIF.
  ...
  METHOD on_show_cart.
    ...
    CONSTANTS: lc_text    TYPE char90 VALUE 'Es sind keine Waren im Warenkorb vorhanden'     ##no_text,
               ...
               lc_butcsd1 TYPE char15 VALUE 'OK'                                             ##no_text.

    IF me->mo_home_screen_model->mt_cart IS NOT INITIAL.

      IF mo_cart_view IS NOT BOUND.
        mo_cart_view = NEW zcl_csd_cart_view( io_cntrl = me ).
      ENDIF.
  ...
  METHOD search_selected_item_in_cart.
    ...
    TRY.
        ...
        IF sy-subrc <> 0.
          MESSAGE i044(zcsd_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD search_selected_product.
    ...
    TRY.
        ...
        IF ls_product IS INITIAL.
          MESSAGE i043(zcsd_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD send_order_confirmation.
    ...
    TRY.
        ...
        IF lo_send_request->send( i_with_error_screen = abap_true ) = abap_false.
          ...
          MESSAGE i061(zcsd_web_shop) INTO DATA(ls_msg).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
      ...
      CATCH zcx_csd_webshop_exception_new
            cx_send_req_bcs
            cx_address_bcs
  ...
  METHOD show_article.
    ...
    IF mo_custom_container_article IS NOT BOUND.
      ...
      mo_alv_grid_article->set_table_for_first_display( EXPORTING i_structure_name = 'ZCSD_ARTICLE'
    ...
    TRY.
        ...
        CALL FUNCTION 'DP_PUBLISH_WWW_URL'
          ...
          EXCEPTIONS
            ...
            OTHERS                = 4.
        IF sy-subrc <> 0.
          MESSAGE: i063(zcsd_web_shop) WITH sy-subrc INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_logo_home_screen->load_picture_from_url_async( EXPORTING    url    = lv_url
                                                              EXCEPTIONS   error  = 1
                                                                           OTHERS = 2 ).
        IF sy-subrc <> 0.
          MESSAGE: i064(zcsd_web_shop) WITH sy-subrc INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_logo_home_screen->set_display_mode( EXPORTING   display_mode = cl_gui_picture=>display_mode_fit
                                                   EXCEPTIONS  error        = 1
                                                               OTHERS       = 2 ).
        IF sy-subrc <> 0.
          MESSAGE: i065(zcsd_web_shop) WITH sy-subrc INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD show_order.
    ...
    IF it_order IS NOT INITIAL.
      IF me->mo_container_show_position IS NOT BOUND.
        ...
        SET HANDLER me->add_butcsds_show_position FOR me->mo_alv_grid_show_position.
        SET HANDLER me->on_butcsds_show_position  FOR me->mo_alv_grid_show_position.
        SET HANDLER me->on_double_click_position  FOR me->mo_alv_grid_show_position.

        me->mo_alv_grid_show_position->set_table_for_first_display( EXPORTING i_structure_name = 'ZCSD_ORDER'
    ...
    ELSE.
      MESSAGE i068(zcsd_web_shop) INTO  DATA(lv_msg).
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
      RAISE EXCEPTION TYPE zcx_csd_webshop_exception_new USING MESSAGE.
    ENDIF.
